...
export default function AnimatedBackground() {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none z-0">
      ...
      <div className="absolute top-[110vh] left-0 w-full h-[150px] z-10 opacity-30">
      ...
      {/* LAYER 4: Front Floating Particles (Ambient Motion via Keyframe CSS) */}
      <div className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none z-10">
        ...
        {[...Array(12)].map((_, i) => {
          const top = `${10 + (i * 7.5)}%`;
          const left = `${5 + (Math.sin(i) * 4 + 4) * 10}%`;
          const size = `${4 + (i % 3) * 2}px`;
          const delay = `${i * -1.2}s`;
          return (
            <div
              key={`part-${i}`}
              className="absolute ambient-particle rounded-full bg-[var(--theme-primary)]"
              style={{
                top,
                left,
                width: size,
                height: size,
                opacity: prefersReduced ? 0 : 0.05,
