...
def test_chats_and_notifications_flow():
    ...
    try:
        ...
        switch_fail = client.post("/auth/switch-role", headers=doc_headers)
        assert switch_fail.status_code == 403 # forbidden
        
        # Approve admin permissions for doctor
        db = TestingSessionLocal()
        doc_db_user = db.query(models_import().User).filter_by(id=doc_user.id).first()
        doc_db_user.has_admin_permission = True
        db.commit()
        
        # Manually try to set patient has_admin_permission = True and test if switch is blocked
        pat_db_user = db.query(models_import().User).filter_by(id=pat_user.id).first()
        pat_db_user.has_admin_permission = True
        db.commit()
        db.close()

        # Patient role switch should still fail (403)
        pat_switch_fail = client.post("/auth/switch-role", headers=pat_headers)
        assert pat_switch_fail.status_code == 403

        # Now switch doctor role (should switch from doctor to admin)
        switch_ok = client.post("/auth/switch-role", headers=doc_headers)
        assert switch_ok.status_code == 200
        assert switch_ok.json()["role"] == "admin"

        # Switch back (should switch from admin to doctor)
        switch_back = client.post("/auth/switch-role", headers=doc_headers)
        assert switch_back.status_code == 200
        assert switch_back.json()["role"] == "doctor"
    finally:
        try:
            next(generator)
